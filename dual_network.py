...
def load_model(path: str, device: torch.device | None = None) -> DualNetwork:
    """
    Load a checkpoint from *path*.  Architecture is inferred automatically
    from the saved weight shapes — no need to match constructor arguments.
    """
...
if __name__ == "__main__":
    ...
    model = DualNetwork(boardsize=N, filters=64, num_residual=6).to(DEVICE)
    total = sum(p.numel() for p in model.parameters())
    print(f"Parameters: {total:,}")
    print(f"Action space: {action_space_size(N)}")
    ...
    with torch.no_grad():
        ...
        pl, v = model(batch)
        dt = (time.perf_counter() - t0) * 1000
    print(f"Batch B={B}: {dt:.1f} ms  output shapes: {tuple(pl.shape)}, {tuple(v.shape)}")
